...
- (MLEmojiLabel *)emojiLabel
{
	if (!_emojiLabel) {
        ...
        _emojiLabel.backgroundColor = [UIColor colorWithRed:0.001 green:0.467 blue:0.001 alpha:1.000];
...
- (void)setEmojiText:(NSString *)emojiText
{
    ...
    self.emojiLabel.emojiText = emojiText;
...
- (void)layoutSubviews
{
    ...
    self.emojiLabel.frame = CGRectMake(10.0f, 5.0f, self.frame.size.width-10.0f*2, self.frame.size.height-5.0f*2);
}
...
//        protypeLabel.customEmojiPlistName = @"expressionImage_custom.plist";
    ...
    [protypeLabel setEmojiText:emojiText];
    return [protypeLabel preferredSizeWithMaxWidth:300.0f].height+5.0f*2;
}
